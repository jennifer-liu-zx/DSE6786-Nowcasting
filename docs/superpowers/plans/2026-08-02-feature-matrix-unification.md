...
> **Correction (post-review):** The `build_X4` bullet below originally claimed the inner-join fix "recovers 18 POOS evaluations" for `RF_Lags_UMIDAS`. The final whole-branch review found this claim did not hold against the real production path: that measurement was taken by feeding raw, unprocessed CSV fixtures directly into `build_X4`, bypassing a step the real POOS pipeline always runs first — `pipeline.poos.cut_and_fill()` calls `pipeline.ragged_edge.fill_ragged_edge_until()`, which interpolates (or zero-fills) every remaining gap, including the `CP3Mx`/`COMPAPFFx` gap referenced below, before any builder ever sees the data. Re-running `fill_ragged_edge_until` on the real fixtures at a cutoff spanning that gap confirms the filled monthly data has zero NaNs at those dates. The inner-join fix itself is still correct and worth keeping (it matches live's behavior and closes a latent hazard for any future caller that skips ragged-edge filling), but it does not change any currently-stored production POOS evaluation numbers. The bullet is left below for historical record of the original (incorrect) justification, with the correction here for anyone reading it going forward.
...
- `build_X4` switches its internal join from `how="left"` (POOS's old behavior) to `how="inner"` (matching live's existing behavior) — this is an intentional defensive fix, not a bug carried forward. ~~It recovers 18 POOS evaluations (2021-09 through 2025-12) for the production `RF_Lags_UMIDAS` model that are currently lost because a single missing raw FRED-MD value (`CP3Mx`/`COMPAPFFx` at 2020-06) poisons every downstream training window that still contains it.~~ *(See correction above: in the real POOS pipeline, `fill_ragged_edge_until()` already closes this gap before `build_X4` runs, so no currently-stored evaluation numbers actually change. The inner join instead closes a latent hazard — it matches live's join semantics exactly and prevents a future caller that skips ragged-edge filling from silently leaking an incomplete row into POOS's windowing logic.)* `build_X4` also gains an explicit `n_gdp_lags: int = 4` parameter (previously hardcoded in the live version).
